...
const resolvers = {
    Query: {
        ...
        getAllFiles: async (parent, { learningId }) => {
            const session = driver.session();
            try {
                const result = await session.run('MATCH (l:Learning)-[:HAS_FILE]->(f:File) WHERE l.id = $learningId RETURN f', { learningId });
        ...
        getLearningById: async (parent, { learningId }) => {
            const session = driver.session();
            try {
                const result = await session.run('MATCH (l:Learning { id: $learningId }) RETURN l', { learningId });
        ...
        getFileById: async (parent, { learningId, fileId }) => {
            const session = driver.session();
            try {
                const result = await session.run('MATCH (l:Learning { id: $learningId })-[:HAS_FILE]->(f:File { id: $fileId }) RETURN f', { learningId, fileId });
                return result.records.map(record => record.get('f').properties);
            }
            finally {
                await session.close();
    ...
    Mutation: {
        ...
        deleteLearning: async (parent, { learningId }) => {
            ...
            try {
                ...
                const result = await session.run('MATCH (l:Learning {id: $learningId}) ' +
                    'DETACH DELETE l', { learningId });
        ...
        updateFile: async (parent, { learningId, fileId, fileText }) => {
            ...
            try {
                const result = await session.run('MATCH (l:Learning { id: $learningId }) ' +
                    ...
                    'RETURN f', { learningId, fileId, fileText });
